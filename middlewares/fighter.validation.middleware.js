...
const updateFighterValid = async (req, res, next) => {
    ...
    for (let key in req.body) {
        key === 'name' && await check('name', 'Укажите имя')
            .notEmpty(trim)
            .run(req);
        key === 'power' && await check('power', "Укажите силу от 1 до 100")
            .isInt({ min: 1, max: 100 })
            .run(req);
        key === 'defense' && await check('defense', "Укажите уровень защиты от 1 до 10")
            .isInt({ min: 1, max: 10 })
            .run(req);
    }
    const errors = validationResult(req);
